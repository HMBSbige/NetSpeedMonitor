...
namespace NetSpeedMonitor.NetUtils
{
	...
	public class NetPacketTool
	{
		...
		public void Start()
		{
			if (m_Monitor == null)
			{
				try
				{
					...
					m_Monitor.BeginReceive(Buffer, 0, Buffer.Length, SocketFlags.None, OnReceive, null);
				}
				catch
				{
					m_Monitor = null;
		...
		private void OnReceive(IAsyncResult ar)
		{
			try
			{
				var received = m_Monitor.EndReceive(ar);
				try
				{
					if (m_Monitor != null)
					{
	...
	/// </summary>
	/// <param name="pm">The <see cref="NetPacketTool"/> that intercepted the <see cref="Packet"/>.</param>
	...
	public class Packet
	{
		...
		/// </remarks>
		public string Source => m_SourcePort != -1 ? $@"{SourceAddress}:{m_SourcePort}" : SourceAddress.ToString();
		...
		public string Destination
		{
			get
			{
				if (m_DestinationPort != -1)
					return DestinationAddress + ":" + m_DestinationPort;
		...
		public string ToString(bool raw)
		{
			...
			else
			{
				...
				{
					for (var j = i; j < Raw.Length && j < i + 16; j++)
					{
